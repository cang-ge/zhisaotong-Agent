...
---
# 使用必看
请务必安装好相关配置环境，其中 `config/agent.yml` 文件中的配置需要根据个人情况修改：
- `gaodekey`：你的高德地图 Web 服务 API Key
- `bailian_api_key`：你的阿里云百炼平台 API Key

> 也可将 API Key 通过环境变量方式配置（详见下方配置说明）
...
本项目使用阿里云通义千问大模型和 DashScope Embedding，需要配置 API Key。
...
**方式二：配置文件**
在 `config/agent.yml` 中配置 `bailian_api_key`：
```yaml
...
```yaml
chat_model_name: qwen3-max          # 对话大模型，可选 qwen3-max、qwen-plus 等
embedding_model_name: text-embedding-v4  # 向量化模型
```

> 模型名称需与阿里云百炼平台支持的模型名称一致。
...
```bash
# 设置阿里云百炼 API Key（方式一：环境变量）
export DASHSCOPE_API_KEY="your_dashscope_api_key"

# 或在 config/agent.yml 中配置（方式二：配置文件）
bailian_api_key: YOUR_BAILIAN_API_KEY

# 配置高德地图 API Key（在 config/agent.yml 中）
gaodekey: YOUR_AMAP_KEY
```
